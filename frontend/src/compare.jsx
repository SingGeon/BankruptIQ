        ...
        {selectedCompanies.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(selectedCompanies.length, 2)}, 1fr)`, gap: "var(--gap)" }}>
              {selectedCompanies.map((c, i) => {
                const zc = window.zColor(c.zscore);
                const riskLbl = c.zscore < 1.81 ? "RISC ÎNALT" : c.zscore < 2.99 ? "RISC MEDIU" : "RISC SCĂZUT";
                return (
                  <section key={c.ticker} className="card">
                    <header className="card-head" style={{ borderBottom: `2px solid ${PALETTE[i]}` }}>
                      ...
                      <span style={{
                        ...
                        borderRadius: 999, background: `color-mix(in oklab, ${zc} 15%, transparent)`,
                        color: zc, fontWeight: 700, border: `1px solid color-mix(in oklab, ${zc} 30%, transparent)`
                      }}>
                        {riskLbl}
                      </span>
                    </header>
                    <div className="card-body">
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 12 }}>
                        {[
                          { l: "Altman Z",   v: c.altmanZ.toFixed(2),   color: zc },
                          { l: "P(falim.)",  v: c.prob12.toFixed(1)+"%", color: window.probColor(c.prob12) },
